...
public class LeerServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String sql = "SELECT id, equipo1, resultado1, resultado2, equipo2 FROM tabla";

            // 4️⃣ Ejecutar la consulta y guardar los datos obtenidos en el ResultSet
            resultados = declaracion.executeQuery(sql);
            try (ResultSet conjutoResultados = declaracion.executeQuery(sql)){
            	request.setAttribute("conjuntoResultados", conjutoResultados);
            	request.getRequestDispatcher("vistas/tabla.jsp").forward(request, response);
            }
          /* 
