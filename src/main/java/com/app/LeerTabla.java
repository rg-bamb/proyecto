...
@WebServlet("/LeerTabla")
public class LeerTabla extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String sql = "SELECT * FROM partido where curso like '1%'";
            // 4️⃣ Ejecutar la consulta y guardar los datos obtenidos en el ResultSet
          

            try (ResultSet conjuntoPartidos = declaracion.executeQuery(sql)) {

                request.setAttribute("conjuntoPartidos", conjuntoPartidos);
                request.getRequestDispatcher("vistas/tablaPartidos.jsp").forward(request, response);
            }
