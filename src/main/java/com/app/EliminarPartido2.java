...
@WebServlet("/eliminarPartido2")
public class EliminarPartido2 extends HttpServlet {
         ...
         try (Connection conexion = ConexionBD.obtenerConexion()) {
             ...
             try (PreparedStatement statement = conexion.prepareStatement(sql)) {
                 ...
                 if (algunaEliminacionExitosa) {
                     response.sendRedirect(request.getContextPath() + "/LeerTabla2"); // Redirigir a la lista de datos
                 } else {
             ...
             try (Connection conexion = ConexionBD.obtenerConexion()) {
                 ...
                 try (PreparedStatement statement = conexion.prepareStatement(sql)) {
                     ...
                     if (filasAfectadas > 0) {
                         ...
                         response.sendRedirect(request.getContextPath() + "/LeerTabla2");
                     } else {
                         response.sendRedirect(request.getContextPath() + "/vistas/error.jsp"); // Redirigir a una página de error
                     }
                 }
